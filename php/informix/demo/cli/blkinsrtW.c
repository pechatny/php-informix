...
*
*
*  Title:          blkinsrtW.c (Unicode counterpart of blkinsrt.c)
*
...
typedef struct 
{
    ...
    SQLLEN       item_num;
    SQLLEN       cbItemNum;
    SQLWCHAR	order_dateW[BUFFER_LEN];
    SQLLEN       cbOrderDate;
    SQLLEN       quantity;
    SQLLEN       cbQuantity;
} orderStruct;
          ...
          char*        argv[])
{
    ...
    */
    ...
    SQLWCHAR *   selectStmtW =  L"SELECT order_num, cust_num, item_num, order_date, quantity FROM orders";
    ...
    */
    ...
    rc = SQLBindCol (hstmt, 4, SQL_C_WCHAR, &orders[0].order_dateW, BUFFER_LENW, &orders[0].cbOrderDate);
    ...
    for (i=0; i<NUM_OLD_ORDERS; i++) 
    {
        ...
        order_date = (SQLCHAR *) malloc (wcslen(orders[i].order_dateW)
                                         + sizeof(char));
        wcstombs( (char *) order_date, orders[i].order_dateW,
                             wcslen(orders[i].order_dateW) + sizeof(char));
        fprintf (stdout, "Order Num: %d Cust Num: %d Item Num: %d Order Date %s Quantity: %d\n", 
                 orders[i].order_num, orders[i].cust_num, orders[i].item_num, order_date, orders[i].quantity);
    }
	...
	*/
    ...
    for (i=0; i<NUM_NEW_ORDERS; i++)
    {
        ...
        wcscpy ((SQLWCHAR *) orders[NUM_OLD_ORDERS+i].order_dateW, L"1999-03-05");
    ...
    for (i=0; i<NUM_NEW_ORDERS; i++) 
    {
        order_date = (SQLCHAR *) malloc (wcslen(orders[NUM_OLD_ORDERS+i].order_dateW)
                                         + sizeof(char));
        wcstombs( (char *) order_date, orders[NUM_OLD_ORDERS+i].order_dateW,
                             wcslen(orders[i].order_dateW) + sizeof(char));
        /* Display the values */
        fprintf (stdout, "Order Num: %d Cust Num: %d Item Num: %d Order Date %s Quantity: %d\n", 
                 orders[NUM_OLD_ORDERS+i].order_num, orders[NUM_OLD_ORDERS+i].cust_num, orders[NUM_OLD_ORDERS+i].item_num, order_date, orders[NUM_OLD_ORDERS+i].quantity);
    }
