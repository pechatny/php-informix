...
*
*
*  Title:          loinfoW.c (Unicode counterpart of loinfo.c)
*
          ...
          char*        argv[])
{
    ...
    */
    ...
    SQLWCHAR		    sbspace_nameW[BUFFER_LENW];
    SQLINTEGER		sbspace_name_size = SQL_NTS;
    ...
    */
    ...
    rc = SQLGetInfoW (hdbc, SQL_INFX_LO_STAT_LENGTH, &lostat_size,
    ...
    */
    ...
    rc = SQLGetInfoW(hdbc, SQL_INFX_LO_SPEC_LENGTH, &lospec_size,
    ...
    rc = SQLExecDirectW (hstmt, (SQLWCHAR *) L"{call ifx_lo_stat_cspec(?, ?)}",
                         SQL_NTS);
    ...
    */
    ...
    rc = SQLExecDirectW (hstmt, (SQLWCHAR *) L"{call ifx_lo_specget_sbspace(?, ?)}", SQL_NTS);
    if (checkError (rc, SQL_HANDLE_STMT, hstmt, (SQLCHAR *) "Error in Step 8 -- SQLExecDirect failed\n"))
        goto Exit;
    /*
        sbspace_name = (SQLCHAR *) malloc (wcslen(sbspace_nameW)
                                              + sizeof(char));
        wcstombs( (char *) sbspace_name, sbspace_nameW, wcslen(sbspace_nameW)
                                              + sizeof(char));
    */
